...
class CIConfigGenerator:
    ...
    def _global_config(self):
        return (
            ...
            "# maven:3.9-eclipse-temurin-17 includes both Maven and JDK 17.\n"
    ...
    def _consumer_contract_test_job(self, structure):
        ...
        return (
            ...
            f"#\n"
            f"# What happens:\n"
            f"#   1. Stub Runner loads the Provider's stubs JAR\n"
            f"#   2. Starts a WireMock server with stub responses\n"
            f"#   3. Consumer's UserServiceClient calls WireMock\n"
            f"#   4. Tests verify the Consumer correctly parses responses\n"
            f"#\n"
            f"# This job depends on provider-contract-test because it\n"
            f"# needs the stubs JAR generated by the Provider build.\n"
            f"# ============================================================\n"
            ...
            f"      artifacts: true\n"
            f"  script:\n"
            f"    # Install Provider stubs to local Maven repo so Stub Runner can find them\n"
            f"    - mvn $MAVEN_CLI_OPTS install:install-file\n"
            f"        -Dfile={pd}/target/provider-api-1.0.0-SNAPSHOT-stubs.jar\n"
            f"        -DgroupId=com.contracttest\n"
            f"        -DartifactId=provider-api\n"
            f"        -Dversion=1.0.0-SNAPSHOT\n"
            f"        -Dpackaging=jar\n"
            f"        -Dclassifier=stubs\n"
            f"    - cd {cd}\n"
    ...
    def _ai_agent_drift_job(self, structure):
        ...
        return (
            ...
            f"  stage: test\n"
            f"  image: maven:3.9-eclipse-temurin-17\n"
            f"  needs:\n"
            f"    - job: provider-contract-test\n"
            f"      artifacts: true\n"
            f"  before_script:\n"
    ...
    def _deploy_job(self, structure):
        return (
            ...
            "    # Examples:\n"
            "    #   - docker build -t provider-api:$CI_COMMIT_SHA provider-api/\n"
            "    #   - docker push registry.example.com/provider-api:$CI_COMMIT_SHA\n"
            "    #   - kubectl set image deployment/provider provider=registry.example.com/provider-api:$CI_COMMIT_SHA\n"
            "  rules:\n"
